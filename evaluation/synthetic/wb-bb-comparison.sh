                ...
                echo "    Acceleration $acc:"
                    for i in $(seq 1 $REPETITIONS); do
                        ...
                        if [[ "$acc" = "0" ]]; then
                            ...
                            "$TIME_COMMAND" -f "%e;%M" -o "$BATCH_TIME_REPORT" "$WORK_DIR/monitor-whitebox.sh" --in "$INPUT_FILE" --out "$VERDICT_FILE" --formula "$WORK_DIR/synthetic/$formula.mfotl" --processors $numcpus --job "$JOB_NAME" > "$JOB_REPORT"
			    wait
