...
MULTISOURCE_VARIANTS="1 2"
EVENT_RATES="500000 700000 900000"
MAX_OOO="1"
WATERMARK_PERIOD="1"
ACCELERATIONS="1.0"
INDEX_RATES="1 2000 4000"
NUM_INPUTS="1 2 4"
PROCESSORS="16"
MONPOLY_CPU_LIST="0"
                    ...
                    for ir in $INDEX_RATES; do
                        for maxooo in $MAX_OOO; do
                            for wmperiod in $WATERMARK_PERIOD; do
                                ...
                                INPUT_FILE="$OUTPUT_DIR/gen_${formula}_${er}_${ir}.csv"
                                "$WORK_DIR"/trace-transformer.sh --seed 5747549845 -v $variant -n $numsources --watermark_period $wmperiod --max_ooo $maxooo -o "$EXEC_LOG_DIR/preprocess_out" "$INPUT_FILE"
