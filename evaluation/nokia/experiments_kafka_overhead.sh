                ...
                for acc in $ACCELERATIONS; do
                    ...
                    for cmd in $MONPOLY_CMDS; do
                        ...
                        for i in $(seq 1 $REPETITIONS); do
                            ...
                            JOB_NAME="nokia_flink_monpoly_${numsources}_${procs}_${cmd_string}_${variant}_${formula}_${acc}_${reorder}_1_${i}"
                            ...
                            rm -r "$VERDICT_FILE" 2> /dev/null
                            "$WORK_DIR/replayer.sh" -o kafka -v $(variant_replayer_params $variant) --other_branch -n $numsources -a $acc -q $REPLAYER_QUEUE -i csv -f csv -t 1000 "$EXEC_LOG_DIR/preprocess_out" 2> "$DELAY_REPORT"&
