...
for variant in $MULTISOURCE_VARIANTS; do
    for numsources in $KAFKA_PARTS; do
        ...
        PREPROCESSED_FILE="$STATE_DIR/preprocessed_$variant_$numsources"
        if [[ -a "$PREPROCESSED_FILE" ]]; then
            echo "alread preprocessed, skipping"
        else
            "$WORK_DIR"/trace-transformer.sh -v $variant -n $numsources -o "$PREPROCESSED_FILE" "$ROOT_DIR/ldcc_sample.csv"
        fi
    done
done
                        ...
                        for acc in $ACCELERATIONS; do
                            echo "                  Acceleration $acc:"
                                for i in $(seq 1 $REPETITIONS); do
                                    ...
                                    "$TIME_COMMAND" -f "%e;%M" -o "$BATCH_TIME_REPORT" "$WORK_DIR/monitor.sh" --in $(inp_type_flink_args "$inp_type") --format csv --out "$VERDICT_FILE" --monitor monpoly --command "$TIME_COMMAND -f %e;%M -o $TIME_REPORT $cmd -nonewlastts $NEGATE" --sig "$WORK_DIR/nokia/ldcc.sig" --formula "$WORK_DIR/nokia/$formula.mfotl" --processors $procs --queueSize "$FLINK_QUEUE" --job "$JOB_NAME" --multi $variant --clear false --nparts $numsources $(monpoly_cmd_to_flink_args "$MONPOLY_CMD" "$STATE_FILE") $(reorder_to_flink_args "$reorder")  > "$JOB_REPORT"
                                    wait
                                done # reps
                        done # acc
                    done # formula
