...
for formula in $FORMULAS; do
    ...
    for acc in $ACCELERATIONS; do
        ...
        for i in $(seq 1 $REPETITIONS); do
            ...
            (taskset -c $AUX_CPU_LIST "$WORK_DIR/replayer.sh" -v -a $acc -q $REPLAYER_QUEUE -i csv -f monpoly "$WORK_DIR/ldcc_sample.csv" 2> "$DELAY_REPORT") \
...
for formula in $FORMULAS; do
    ...
    for acc in $ACCELERATIONS; do
        ...
        for i in $(seq 1 $REPETITIONS); do
            ...
            rm "$VERDICT_FILE" 2> /dev/null
            (taskset -c $AUX_CPU_LIST "$WORK_DIR/replayer.sh" -v -a $acc -q $REPLAYER_QUEUE -i csv -f monpoly "$WORK_DIR/ldcc_sample.csv" 2> "$DELAY_REPORT" | cut -d " " -f2) \
    ...
    for formula in $FORMULAS; do
        ...
        for acc in $ACCELERATIONS; do
            ...
            for i in $(seq 1 $REPETITIONS); do
                ...
                rm "$VERDICT_FILE" 2> /dev/null
                taskset -c $AUX_CPU_LIST "$WORK_DIR/replayer.sh" -v -a $acc -q $REPLAYER_QUEUE -i csv -f csv -t 1000 -o localhost:$STREAM_PORT "$WORK_DIR/ldcc_sample.csv" 2> "$DELAY_REPORT" &
                "$TIME_COMMAND -f %e;%M -a -o $TIME_REPORT" "$WORK_DIR/monitor.sh" --in localhost:$STREAM_PORT --format csv --out "$VERDICT_FILE" --monitor monpoly --command "$TIME_COMMAND -f %e;%M -a -o $TIME_REPORT $MONPOLY_EXE -negate" --sig "$WORK_DIR/nokia/ldcc.sig" --formula "$WORK_DIR/nokia/$formula.mfotl" --processors $numcpus --job "$JOB_NAME" > "$JOB_REPORT"
                wait
                ...
                rm "$VERDICT_FILE" 2> /dev/null
                taskset -c $AUX_CPU_LIST "$WORK_DIR/replayer.sh" -v -a $acc -q $REPLAYER_QUEUE -i csv -f csv -t 1000 -o localhost:$STREAM_PORT "$WORK_DIR/ldcc_sample.csv" 2> "$DELAY_REPORT" &
                "$TIME_COMMAND -f %e;%M -a -o $TIME_REPORT" "$WORK_DIR/monitor.sh" --in localhost:$STREAM_PORT --format csv --out "$VERDICT_FILE" --monitor dejavu --command "$TIME_COMMAND -f %e;%M -a -o $TIME_REPORT $DEJAVU_EXE" --sig "$WORK_DIR/nokia/ldcc.sig" --formula "$WORK_DIR/nokia/$formula.mfotl" --processors $numcpus --job "$JOB_NAME" > "$JOB_REPORT"
                wait
            done
        done
    done
