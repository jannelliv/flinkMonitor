        ...
        for acc in $ACCELERATIONS; do
            ...
            for i in $(seq 1 $REPETITIONS); do
                ...
                if [[ "$acc" = "0" ]]; then
                    ...
                    rm -r "$VERDICT_FILE" 2> /dev/null
                    "$TIME_COMMAND" -f "%e;%M" -o "$BATCH_TIME_REPORT" "$WORK_DIR/monitor.sh" --in "$ROOT_DIR/ldcc_sample_linear.dvu" --format dejavu --out "$VERDICT_FILE" --monitor dejavu --command "$TIME_COMMAND -f %e;%M -o $TIME_REPORT $DEJAVU_EXE" --sig "$WORK_DIR/nokia/ldcc.sig" --formula "$WORK_DIR/nokia/$formula.mfotl" --processors $numcpus --queueSize "$FLINK_QUEUE" --job "$JOB_NAME" > "$JOB_REPORT"
                    wait
        ...
        for acc in $ACCELERATIONS; do
            ...
            for i in $(seq 1 $REPETITIONS); do
                ...
                if [[ "$acc" = "0" ]]; then
                    ...
                    rm -r "$VERDICT_FILE" 2> /dev/null
                    "$TIME_COMMAND" -f "%e;%M" -o "$BATCH_TIME_REPORT" "$WORK_DIR/monitor.sh" --in "$ROOT_DIR/ldcc_sample_linear.dvu" --format dejavu --out "$VERDICT_FILE" --monitor dejavu --command "$TIME_COMMAND -f %e;%M -o $TIME_REPORT $DEJAVU_EXE" --sig "$WORK_DIR/nokia/ldcc.sig" --formula "$WORK_DIR/nokia/$formula.mfotl" --processors $numcpus --heavy "$HEAVY_FILE" --queueSize "$FLINK_QUEUE" --job "$JOB_NAME" > "$JOB_REPORT"
                    wait
