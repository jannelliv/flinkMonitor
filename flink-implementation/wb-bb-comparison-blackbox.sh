...
ACCELERATIONS="1"
INDEX_RATES="1"
PROCESSORS="5/0-6,24-30 10/0-11,24-35 15/0-16,24-40 20/0-21,24-45"
MONPOLY_CPU_LIST="0"
...
make_log() {
    ...
    for er in $EVENT_RATES; do
        for ir in $INDEX_RATES; do
            ...
            "$WORK_DIR/replayer.sh" -v -a 0 -q $REPLAYER_QUEUE -i csv -f csv "$OUTPUT_DIR/gen_${formula}_${er}_${ir}.csv" > "$OUTPUT_DIR/gen_${formula}_${er}_${ir}.log"
        done
    done
}
    ...
    for formula in $FORMULAS; do
        ...
        for er in $EVENT_RATES; do
            ...
            echo "      Event rate $er, index rate $ir:"
			    ...
			    taskset -c $AUX_CPU_LIST "$WORK_DIR/replayer.sh" -v -a $acc -q $REPLAYER_QUEUE -i csv -f csv -o localhost:$STREAM_PORT "$INPUT_FILE" 2> "$DELAY_REPORT" &
                            "$TIME_COMMAND" -f "%e;%M" -o "$BATCH_TIME_REPORT" "$WORK_DIR/monitor.sh" --in localhost:$STREAM_PORT  --format csv --out "$VERDICT_FILE" --command "$TIME_COMMAND -f %e;%M -o $TIME_REPORT $MONPOLY_EXE -nonewlastts $NEGATE" --sig "$WORK_DIR/synthetic/synth.sig" --nparts 1 --formula "$WORK_DIR/synthetic/$formula.mfotl" --processors $numcpus --skipreorder --multi 1 --clear false --queueSize "$FLINK_QUEUE" --job "$JOB_NAME" > "$JOB_REPORT"  
                            wait
